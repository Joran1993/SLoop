-- Voeg aanvraag 5w-6m window toe aan pijplijn tier:
--
--   eindhoven_vergunning  = aanvraag < 5 weken, geen verleend/sloopmelding   (VROEG)
--   pijplijn              = aanvraag 5w-6m óf bestemmingswijziging/MER/etc.  (PIJPLIJN)
--   koop_sloopmelding     = verleend, sloopmelding, of aanvraag > 6 maanden  (LAAT)
--
-- Geeft ~296 extra pijplijn leads: aanvragen die nog in behandeling zijn.
...
classified AS (
  SELECT
    ...
    CASE
      ...
      -- Aanvraag aanwezig: vroeg als < 5 weken, pijplijn als 5w-6m, anders laat
      WHEN COALESCE(sd.has_aanvraag, false)
        THEN CASE
          WHEN (NOW() - sd.earliest_aanvraag) <= INTERVAL '5 weeks'  THEN 'eindhoven_vergunning'
          WHEN (NOW() - sd.earliest_aanvraag) <= INTERVAL '6 months' THEN 'pijplijn'
          ELSE 'koop_sloopmelding'
        END
      -- Geen aanvraag, wel vroeg pijplijn signaal (bestemmingswijziging, MER, etc.)
