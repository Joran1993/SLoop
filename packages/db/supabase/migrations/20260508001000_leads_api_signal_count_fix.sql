-- Fix signal_count to exclude the primary sloopmelding signal (which is the lead source itself).
-- This makes signal_count represent ADDITIONAL intelligence beyond the lead's primary source.
-- Before: 796 leads with signal_count > 0 (most just counting their own sloopmelding)
-- After:  ~179 leads with signal_count > 0 (leads with vergunning, aanvraag, etc.)
