"""Bron 4: Commissie voor de milieueffectrapportage (Commissie m.e.r.) via RSS.

Signaalwaarde: 24-48 maanden vóór sloop/herontwikkeling. Horizon: 24-48 maanden.

RSS feed: https://www.commissiemer.nl/feed
Filtert op projecten met herontwikkeling/sloop-trefwoorden in titel/beschrijving.
"""
...
_SIGNAL_KEYWORDS = [
    ...
    "nieuwbouw", "herstructurering", "stedelijke ontwikkeling",
    "gebiedsontwikkeling", "verdichting", "sanering",
]


class CommissieMerAdapter(PipelineSourceAdapter):
    source_name = "commissie_mer"
    cron_schedule = "0 7 * * 1"

    def fetch_signals(self, since: datetime) -> Iterator[RawSignal]:
        since_str = since.strftime("%Y-%m-%d")
        kw_parts = " OR ".join(f'cql.textAndIndexes="{kw}"' for kw in _SIGNAL_KEYWORDS)
        query = (
            f"dt.modified>={since_str}"
            f" AND cql.textAndIndexes=\"milieueffectrapportage\""
            f" AND ({kw_parts})"
        )
        start = 1
        page_size = 50
    ...
    def parse_signal(self, raw: RawSignal) -> ParsedSignal | None:
        ...
        return ParsedSignal(
            ...
            signal_type="mer_aanmelding",
            signal_strength="medium",
            signal_time=signal_time,
            title=(item.get("titel") or "")[:200] or None,
            description=item.get("beschrijving") or None,
            address_text=None,
            postcode=None,
            ...
            estimated_horizon_months_max=48,
...
def _has_keyword(text: str) -> bool:
    t = text.lower()
    return any(kw in t for kw in _SIGNAL_KEYWORDS)
